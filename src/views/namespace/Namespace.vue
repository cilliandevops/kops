<template>
  <div class="namespace">
        ...
        <div>
          <el-card class="namespace-head-card" shadow="never" :body-style="{ padding: '10px' }">
            <el-row>
              ...
              <el-col :span="6">
                <div>
                  <el-input class="namespace-head-search" clearable placeholder="请输入" v-model="searchInput" />
                  <el-button style="border-radius: 2px" icon="Search" type="primary" plain @click="getNamespaces()"
                    >搜索</el-button
                  >
                </div>
              </el-col>
          ...
          <el-card class="namespace-body-card" shadow="never" :body-style="{ padding: '5px' }">
            ...
            >
              ...
              <el-table-column align="center" prop="status.phase" label="状态">
                <template v-slot="scope">
                  <span :class="[scope.row.status.phase === 'Active' ? 'success-status' : 'error-status']">{{
              ...
              <el-table-column align="center" label="操作" min-width="120">
                <template v-slot="scope">
                  <el-button
                    ...
                    type="primary"
                    plain
                    @click="getNamespaceDetail(scope)"
                    >YAML</el-button
                  >
                  <el-button
                    size="small"
                    ...
                    type="danger"
                    plain
                    @click="handleConfirm(scope, '删除', delNamespace)"
                    >删除</el-button
                  >
                </template>
              </el-table-column>
    ...
    <el-dialog title="YAML信息" v-model="yamlDialog" width="45%" top="5%">
      <JsonEditorVue style="height: 500px" v-model="namespaceDetail" :showBtns="false" :mode="'code'" lang="zh" />
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="yamlDialog = false">取 消</el-button>
          <el-button type="primary" @click="updateNamespace()">更 新</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    getNamespaceDetail(e) {
      ...
      this.getNamespacesData.params.namespace = this.namespaceValue
      httpClient
        .get(this.getNamespacesData.url, { params: this.getNamespacesData.params })
        .then((res) => {
    ...
    updateNamespace() {
      const content = JSON.stringify(this.namespaceDetail)
      this.updateNamespaceData.params.namespace = this.namespaceValue
      this.updateNamespaceData.params.content = content
      httpClient
        .put(this.updateNamespaceData.url, this.updateNamespaceData.params)
        .then((res) => {
    ...
    delNamespace(e) {
      ...
      httpClient
        .delete(this.delNamespaceData.url, { data: this.delNamespaceData.params })
        .then((res) => {
          this.getNamespaces()
          this.$message.success({
            message: res.msg
