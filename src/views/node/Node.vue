<template>
  <div class="node">
        ...
        <div>
          <el-card class="node-head-card" shadow="never" :body-style="{ padding: '10px' }">
            <el-row>
              ...
              <el-col :span="6">
                <div>
                  <el-input class="node-head-search" clearable placeholder="请输入" v-model="searchInput" />
                  <el-button style="border-radius: 2px" icon="Search" type="primary" plain @click="getNodes()"
                    >搜索</el-button
                  >
                </div>
              </el-col>
          ...
          <el-card class="node-body-card" shadow="never" :body-style="{ padding: '5px' }">
            ...
            >
              ...
              <el-table-column align="center" label="规格">
                <template v-slot="scope">
                  <el-tag type="success"
                    >{{ scope.row.status.capacity.cpu }}核{{ specTrans(scope.row.status.capacity.memory) }}G</el-tag
                  >
                </template>
              </el-table-column>
              ...
              <el-table-column align="center" label="操作" min-width="120">
                <template v-slot="scope">
                  ...
                  <el-button
                    ...
                    type="warning"
                    plain
                    @click="handleConfirm(scope, '删除', delIngress)"
                    >详情</el-button
                  >
                </template>
              </el-table-column>
    ...
    <el-dialog title="YAML信息" v-model="yamlDialog" width="45%" top="5%">
      <JsonEditorVue style="height: 500px" v-model="nodeDetail" :showBtns="false" :mode="'code'" lang="zh" />
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="yamlDialog = false">取 消</el-button>
          <el-button type="primary" @click="updateNode()">快速更新</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    getNodes() {
      ...
      httpClient
        .get(this.getNodesData.url, { params: this.getNodesData.params })
        .then((res) => {
    ...
    updateNode() {
      const content = JSON.stringify(this.nodeDetail)
      this.updateNodeData.params.namespace = this.namespaceValue
      this.updateNodeData.params.content = content
      httpClient
        .put(this.updateNodeData.url, this.updateNodeData.params)
        .then((res) => {
    ...
    delNode(e) {
      ...
      httpClient
        .delete(this.delNodeData.url, { data: this.delNodeData.params })
        .then((res) => {
          this.getNodes()
          this.$message.success({
            message: res.msg
